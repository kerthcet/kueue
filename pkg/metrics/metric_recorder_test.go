...
var _ MetricRecorder = &fakePendingWorkloadsRecorder{}

type fakePendingWorkloadsRecorder struct {
	counter int64
}

func (r *fakePendingWorkloadsRecorder) Inc() {
	atomic.AddInt64(&r.counter, 1)
}

func (r *fakePendingWorkloadsRecorder) Dec() {
	atomic.AddInt64(&r.counter, -1)
}

func (r *fakePendingWorkloadsRecorder) Clear() {
	atomic.StoreInt64(&r.counter, 0)
}

func TestInc(t *testing.T) {
	fakeRecorder := fakePendingWorkloadsRecorder{}
...
func TestDec(t *testing.T) {
	fakeRecorder := fakePendingWorkloadsRecorder{counter: 100}
	var wg sync.WaitGroup
	loops := 100
	wg.Add(loops)
	for i := 0; i < loops; i++ {
